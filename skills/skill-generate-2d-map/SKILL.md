...
4. For each visible runtime object, choose exactly one asset strategy:
   - generate a separate transparent asset with `$skill-generate-2d-sprite` or direct `image_gen`
...
Scene hooks are metadata only. Do not generate enemy, boss, NPC, player, projectile, or animation sprites inside `skill-generate-2d-map`; call `$skill-generate-2d-sprite` separately when the game needs those assets.
