...
For `3x3` and `4x4` packs, create a layout-only guide first with `$skill-generate-2d-sprite`:

```bash
python skills/skill-generate-2d-sprite/scripts/make_layout_guide.py \
  --rows <ROWS> \
  --cols <COLS> \
  --cell-width 384 \
  --cell-height 384 \
